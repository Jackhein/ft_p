...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2014/04/23 17:10:42 by tbalea          #+#    #+#               */
/*   Updated: 2015/05/28 15:52:55 by tbalea           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

static int		ft_gnl_realloc(char **line, char *buf, int size, int *len)
{
	char		*temp;
...
static int		ft_read_buf(char **line, t_info *cur, int *len)
{
	...
	{
		if (ft_gnl_realloc(line, cur->start, cur->offset, len))
			return (-1);
		cur->offset = 0;
		cur->start = NULL;
	}
	...
	{
		if (ft_gnl_realloc(line, cur->start, n - cur->start, len))
			return (-1);
